...
void *pop(s_mem_stack_list *pool) {
    ...
    // Este es un workaround rápido para la calculadora, para otros tipos de datos habría que mejorar esta función ya que ahora mismo cuando liberamos memoria se pierde la referencia al dato
    switch (sizeof(*block)) {
        case sizeof(int):
            int temp_data = *(int *)block; 
