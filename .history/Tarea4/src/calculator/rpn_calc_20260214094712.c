...
int *parse_int(char *selected_char, s_mem_stack_list *stack_calc) {
    ...
    *ptr_selected_number = selected_number; // Hacemos que el contenido del apuntador sea el mismo que del número    
    return ptr_selected_number; // No es necesario retornar, sólo que la función está definida así en el .h me da flojera modificarlo, además de que aumenta mi tasa de errores
}
...
int *parse_op(char* selectec_char, s_mem_stack_list *stack_calc) {
    ...
    int second_number = *(int *)pop(stack_calc);
    // Estoy casi seguro que hay que pensar algo para las operaciones con cero
