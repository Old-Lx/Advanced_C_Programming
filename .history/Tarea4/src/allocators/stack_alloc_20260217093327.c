...
void *pop(s_mem_stack_list *pool) {
    ...
    block->next = pool->freeList; // Apuntamos a la lista como próximo bloque
    s_mem_block_stack *next_block = pool->freeList; // Si no hago esto, por alguna razón el apuntador next se perdía
    printf("1 block %p content %d\n", block, *(int *)block);
    printf("next_block %p\n", next_block);
    printf("freelist prev %p\n", pool->freeList->prev);
    printf("freelist next %p\n", pool->freeList->next);
    printf("freelist %p\n\n", pool->freeList);
    
    // Acá se ejecuta la parte de liberar memoria
    pool->freeList = block; // Restauramos el espacio libre de la memoria
    printf("2 block %p content %d\n", block, *(int *)block);
    printf("next_block %p\n", next_block);
    printf("freelist prev %p\n", pool->freeList->prev);
    printf("freelist next %p\n", pool->freeList->next);
    printf("freelist %p\n\n", pool->freeList);
    pool->freeList->next = next_block; // Esta línea es la que pierde el dato
    printf("3 block %p content %d\n", block, *(int *)block);
    printf("next_block %p\n", next_block);
    printf("freelist prev %p\n", pool->freeList->prev);
    printf("freelist next %p\n", pool->freeList->next);
    printf("freelist %p\n\n", pool->freeList);
