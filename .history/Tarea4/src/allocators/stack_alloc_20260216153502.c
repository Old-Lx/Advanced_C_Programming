...
void *pop(s_mem_stack_list *pool) {
    ...
    s_mem_stack_list *block = (void *)pool->freeList->prev; // guardaremos el bloque que se quitará del pool
    s_mem_stack_list *next_block = (void *)pool->freeList->next->prev; // Si no hago esto, por alguna razón el apuntador next se perdía
