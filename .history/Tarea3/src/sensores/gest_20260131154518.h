...
typedef struct S_GESTURE {
    union { // la unión permite leer distintos tipos de datos en un sólo espacio de memoria 
        uint8_t tap; // el bitfield limita los bits a usar de un espacio allocated de memoria, este usa el primer bit
        uint8_t double_tap; // este usa el segundo bit
        uint8_t haptic_press;
        uint8_t saved; // Acá se reservan
    } data;
} s_gesture;
