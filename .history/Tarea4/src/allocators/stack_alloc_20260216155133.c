...
void *pop(s_mem_stack_list *pool) {
    ...
    printf("pool before pop %p\n", (int *)pool->pool);
    
    // Ojo acá que de repente da bug al hacer pop del primer elemento
    s_mem_block_stack *block = (void *)pool->freeList->prev; // guardaremos el bloque que se quitará del pool
    s_mem_block_stack *next_block = (void *)pool->freeList->next->prev; // Si no hago esto, por alguna razón el apuntador next se perdía
    pool->freeList = (void *)pool->freeList->prev; // Restauramos el espacio libre de la memoria
    pool->freeList->next = (void *)next_block;
    pool->qtty--;

    printf("popped block %p\n", (int *)block);
