...
void* allocate(s_mem_free_list *pool) {
    ...
    if (!pool) {
        pool->freeList = (s_mem_block_linear *)pool->pool; // Ponemos todo el pool en la lista
        s_mem_block_linear *current = pool->freeList; // se usará para iterar y que se divida exactamente en los bloques definidos

        // creamos la lista
        for (int i = 0; i < (STACK_POOL_SIZE / sizeof(s_mem_block_linear)) - 1; i++) {
            // Si estamos en el índice 0, no guardamos nada en prev
            if (i) {
                current->prev = (s_mem_block_linear *)((unsigned char *)current - sizeof(s_mem_block_linear));
            }
            current->next = (s_mem_block_linear *)((unsigned char *)current + sizeof(s_mem_block_linear));
...
void *free(s_mem_free_list *pool, void *ptr) {
    s_mem_block_linear *block = (s_mem_block_linear *) ptr; // El void *ptr será el bloque que estaremos liberando
...
void *pop(s_mem_free_list *pool) {
    ...
    s_mem_block_linear *block = pool->freeList->prev; // guardaremos el bloque que se quitará del pool
