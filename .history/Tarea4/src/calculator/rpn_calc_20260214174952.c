...
void push_int(int selected_number, s_mem_stack_list *stack_calc) {
    printf("number: %d pointer: %p\n", selected_number, stack_calc->freeList);
    ...
    if (first_number_block && second_number_block) {
        switch (which_op(selectec_char)) {
            case ADD:
                ...
                printf("pointer %p its value %d", stack_calc->freeList->prev, *(int *)stack_calc->freeList->prev);
                break;
            case SUB:
